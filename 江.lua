...
local Button = Tab:Button({
    ...
    Callback = function()
                    ...
                    if mapFolder then
                        for _, g in ipairs(mapFolder:GetChildren()) do
                            if g.Name == "Generator" and g:FindFirstChild("Main") then
                                createEspText(g.Main, "发电机 进度: " .. g.Progress.Value, colors.Generator)
                                
                                local progressConn = g.Progress.Changed:Connect(function()
                                    if textLabels[g.Main] then
                                        textLabels[g.Main].TextLabel.Text = "发电机 进度: " .. g.Progress.Value
                                    end
                                end)
                        ...
                        table.insert(connections, mapFolder.ChildAdded:Connect(function(child)
                            if child.Name == "Generator" and child:FindFirstChild("Main") then
                                createEspText(child.Main, "发电机 进度: " .. child.Progress.Value, colors.Generator)
                                
                                local progressConn = child.Progress.Changed:Connect(function()
                                    if textLabels[child.Main] then
                                        textLabels[child.Main].TextLabel.Text = "发电机 进度: " .. child.Progress.Value
                                    end
                                end)
                                table.insert(progressConnections, progressConn)
                            end
                        ...
                        if obj:IsA("Model") and obj.Name == item then
                            for _, child in ipairs(obj:GetChildren()) do
                                if child:IsA("BasePart") then
                                    createEspText(child, "物品:"..obj.Name, colors.Item)
                                    break
                                end
                            end
                        end
                    ...
                    table.insert(connections, workspace.Map.Ingame.DescendantAdded:Connect(function(descendant)
                        if descendant:IsA("Model") and table.find(Items, descendant.Name) then
                            for _, child in ipairs(descendant:GetChildren()) do
                                if child:IsA("BasePart") then
                                    createEspText(child, "物品:"..descendant.Name, colors.Item)
                                    break
                                end
                            end
                        end
        ...
        -- 确保 Sprinting 变量存在
        local Sprinting = game:GetService("Players").LocalPlayer.Character:FindFirstChild("Sprinting")
            ...
            Callback = function(state)
                ...
                if state then
                    while CoinFlipping and task.wait(1) do
                        ...
                        if currentCharges and currentCharges < WantedCharges then
                            -- 确保 BlockRemote 变量存在
        ...
        local Toggle4 = Tab:Toggle({
            ...
            Callback = function(state)
                autoxiujiT = state
                if state then
                    while autoxiujiT and task.wait(SkibidiWait) do
